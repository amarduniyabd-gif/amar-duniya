...
export default function CategoryPage() {
  ...
  const loadPosts = useCallback(async (pageNum: number, reset: boolean = false) => {
    ...
    try {
      ...
      const filtered = localPosts.filter((p: any) => p.category === currentCategory.id);
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <Link href="/" className="bg-[#f85606] text-white px-6 py-2 rounded-lg inline-block">হোমে ফিরে যান</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gray-100 pb-20">
        ...
        <div className="max-w-2xl mx-auto p-4">
          <div className="bg-gradient-to-br from-pink-50 to-rose-50 rounded-xl shadow-sm p-6 text-center border border-pink-100">
            ...
            <h2 className="text-lg font-semibold text-gray-800 mb-2">পাত্র-পাত্রী বিভাগে স্বাগতম</h2>
            <p className="text-gray-500 text-sm mb-6">এখানে আপনি পাত্র বা পাত্রী খুঁজতে পারবেন। ছবি দেখতে ছোট একটি চার্জ প্রযোজ্য হবে।</p>
            <Link href="/category/matrimony" className="inline-block bg-gradient-to-r from-[#f85606] to-orange-500 text-white px-6 py-3 rounded-xl font-semibold shadow-md hover:shadow-lg transition">পাত্র-পাত্রী দেখুন</Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-20">
      <div className="bg-white px-4 py-3 sticky top-0 z-10 border-b shadow-sm flex items-center gap-3">
        <button onClick={() => router.back()} className="p-1 active:scale-95 transition"><ArrowLeft size={24} className="text-gray-600" /></button>
        <h1 className="text-xl font-bold text-[#f85606]">{currentCategory.name}</h1>
      ...
      <div className="max-w-6xl mx-auto p-4">
        {subCategories.length > 0 && (
          <>
            <div className="flex items-center gap-2 mb-4"><Grid3x3 size={18} className="text-[#f85606]" /><h2 className="text-sm font-medium text-gray-500">সাব-ক্যাটাগরি</h2></div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
              {subCategories.map((subCat: any) => (
                <Link key={subCat.id} href={`/category/${subCat.slug}`} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-all group text-center">
                  <div className="text-3xl mb-2 group-hover:scale-110 transition-transform">{subCat.icon}</div>
                  <h3 className="font-semibold text-gray-800 text-xs group-hover:text-[#f85606]">{subCat.name}</h3>
                </Link>
              ))}
            </div>
          </>
        )}
        <div className="flex items-center justify-between mb-4"><h2 className="text-sm font-medium text-gray-500 flex items-center gap-2"><span>📦</span> সব পোস্ট</h2></div>
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">{[...Array(8)].map((_, i) => <SkeletonCard key={i} />)}</div>
        ) : posts.length > 0 ? (
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">{posts.map((post) => <PostCard key={post.id} post={post} />)}</div>
            {loadingMore && <div className="flex justify-center py-4"><Loader2 className="animate-spin text-[#f85606]" size={24} /></div>}
            {!hasMore && posts.length > 0 && <div className="text-center py-6"><p className="text-xs text-gray-400">🎉 সব পোস্ট দেখানো হয়েছে</p></div>}
          </>
        ) : (
          <div className="text-center py-10 bg-white rounded-xl shadow-sm">
            ...
            <Link href={`/post-ad?category=${currentCategory.id}`} className="bg-[#f85606] text-white px-6 py-3 rounded-xl inline-block font-semibold shadow-md hover:shadow-lg transition">প্রথম পোস্ট দিন</Link>
          </div>
        )}
        <div className="mt-8 pt-4 border-t border-gray-200">
          <Link href="/post-ad" className="flex items-center justify-center gap-2 text-gray-500 text-sm hover:text-[#f85606] transition-colors"><span>➕</span><span>আপনার পণ্যের ক্যাটাগরি এখানে নেই? নতুন ক্যাটাগরিতে পোস্ট করুন</span></Link>
        </div>
      </div>
    </div>
  );

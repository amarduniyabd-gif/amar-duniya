...
export const createProfile = async (userId: string, profile: {
  name?: string; phone?: string; avatar?: string; location?: string; bio?: string;
}) => {
  const supabase = getClient();
  const { data, error } = await supabase.from('profiles').insert({
    id: userId, ...profile, created_at: new Date().toISOString(),
  }).select().single();
  return { data, error };
};
...
export const isVerified = async (): Promise<boolean> => {
  ...
  return (data as any)?.is_verified || false;
};

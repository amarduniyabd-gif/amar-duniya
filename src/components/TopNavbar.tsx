...
function TopNavbarContent() {
  ...
  return (
    <nav className="hidden md:block fixed top-0 left-0 right-0 z-50 bg-white/85 backdrop-blur-xl border-b border-gray-100/50 shadow-sm" suppressHydrationWarning>
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-2.5">
        <div className="flex justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="hidden lg:block">
                <h1 className="text-xl font-black tracking-tight leading-none">
                  <span className="bg-gradient-to-r from-[#f85606] via-orange-500 to-[#ff6b35] bg-clip-text text-transparent">আমার দুনিয়া</span>
                </h1>
                <p className="text-[8px] font-semibold tracking-widest text-gray-400 uppercase">স্বপ্নের ঠিকানা</p>
              </div>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-1.5 p-1.5 rounded-2xl bg-gray-100/60 backdrop-blur-sm">
            {navItems.map((item) => (
              <NavItem key={item.href} href={item.href} icon={item.icon} label={item.label} isActive={isActive(item.href)} badge={item.badge} />
            ))}
          </div>

          <div className="flex items-center gap-3 shrink-0">
            {/* ✅ পোস্ট দিন বাটন - সবসময় দেখাবে */}
            <Link href="/post-ad">

...
function BottomNavContent() {
  ...
  // চ্যাট পেজে নিচের বার দেখাবে না
  if (pathname?.startsWith('/chat')) return null;

  return (
    <footer className="md:hidden fixed bottom-0 left-0 right-0 border-t shadow-[0_-4px_20px_rgba(0,0,0,0.05)] px-2 py-2 z-50 bg-white border-gray-100">
      <div className="flex justify-around items-center max-w-md mx-auto">
        
        <Link href="/" className="flex flex-col items-center">
          <Home size={22} className={isActive("/") ? "text-[#f85606]" : "text-gray-500"} />
          <span className={`text-[10px] mt-1 font-bold ${isActive("/") ? "text-[#f85606]" : "text-gray-500"}`}>
            হোম
          </span>
        </Link>
        ...
        <Link href="/auction" className="flex flex-col items-center">
          ...
          <span className={`text-[10px] mt-1 font-bold ${isActive("/auction") ? "text-[#f85606]" : "text-gray-500"}`}>
            নিলাম
          </span>
        </Link>
        ...
        <Link href="/chat" className="flex flex-col items-center relative">
          ...
          <span className={`text-[10px] mt-1 font-bold ${isActive("/chat") ? "text-[#f85606]" : "text-gray-500"}`}>
            চ্যাট
          </span>
          <span className="absolute -top-1 right-0 h-2 w-2 bg-red-500 rounded-full border-2 border-white"></span>
        </Link>
        ...
        <Link href="/my-account" className="flex flex-col items-center">
          ...
          <span className={`text-[10px] mt-1 font-bold ${isActive("/my-account") ? "text-[#f85606]" : "text-gray-500"}`}>
            অ্যাকাউন্ট
          </span>
        </Link>
...
export default function BottomNav() {
  return (
    <Suspense fallback={<div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-100"></div>}>
      <BottomNavContent />
    </Suspense>
  );
}